...
def singleboard_datareader(arduino_board_port, arduino_board_number):
    ...
    with open(f"alldata_attempt2/sensor_data{arduino_board_number}.csv", "a") as f:
        with serial.Serial() as ser:
            ...
            beginning_of_cycle_tracker = False
            while collect_data:  # sampling time
                for j in range(50):
                    for k in range(1):
                        ...
                        if t_sample_start > 0.5 and beginning_of_cycle_tracker:
                            beginning_of_cycle_tracker = False
                        if t_sample_start < 0.5 and not beginning_of_cycle_tracker:
                            beginning_of_cycle_tracker = True
                            print("new cycle")
...
def calculate_error_threshold(distance, tcp_lifting=1):
    ...
    if 60 < reduced_percentage <= 80:
        ...
        return 60 * tcp_lifting
    elif 20 < reduced_percentage <= 40:
        return 40 * tcp_lifting
...
def sensor_error_queue(
    ...
    tcp_speed = list(state.target_TCP_speed)
    # print(tcp_speed[2])
    ...
    for i in range(2):
        ...
        if min(err) > 0.3:
            ...
            tcp_lifting = 0.5 if tcp_speed[2] > 0.0 else 1
