...
def ax2_fcn(x):
    noise = PerlinNoise(octaves=.1*np.sqrt(2), seed=3)
    if(isinstance(x,int) or isinstance(x,float)): return noise(x)
    fcn = [noise(i) for i in x]
    return np.array(fcn)

def ax3_fcn(x):
    noise = PerlinNoise(octaves=.1*np.sqrt(3), seed=4)
    if(isinstance(x,int) or isinstance(x,float)): return noise(x)
    fcn = [noise(i) for i in x]
    return np.array(fcn)
